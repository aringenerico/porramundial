    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        <span className="award-trigger-val">
          {value || (players.length === 0 ? 'Loading players…' : `Select player`)}
        </span>
      ...
      {open && (
        <div className="award-dropdown" role="listbox">
          ...
          <div className="award-list">
            {filtered.length === 0
              ? <div className="award-empty">No results for "{q}"</div>
              : filtered.map(p => (
                <button
                  key={p.id}
                  type="button"
    ...
    <div className="page">
      <div className="hero">
        ...
        <div className="hero-grid">
          <div className="hero-stat">
            ...
            <div className="hero-stat-lbl">Participants</div>
          </div>
          <div className="hero-stat">
            <div className="hero-stat-val" style={{ color: 'var(--gold)' }}>€{pot}</div>
            <div className="hero-stat-lbl">Total pot</div>
          </div>
          <div className="hero-stat">
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {[
            ...
            { n:'2', t:'Make award predictions',   d:'Top Scorer, MVP, Best Young Player & Goalkeeper (+10 pts each)' },
      ...
      <div className="card" style={{ background:'linear-gradient(135deg,#0e1e38,#091428)' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, textAlign:'center' }}>
          {[
            { lbl:'1st Prize', pct:'75%', col:'var(--gold)', medal:'🥇', amt:Math.round(pot*0.75) },
            { lbl:'2nd Prize', pct:'20%', col:'#c0c8d8',     medal:'🥈', amt:Math.round(pot*0.20) },
            { lbl:'3rd Prize', pct:'5%',  col:'#a07040',     medal:'🥉', amt:Math.round(pot*0.05) },
          ].map(p => (
    ...
    <div className="page">
        ...
        {Object.entries(GROUPS).map(([key,g]) => (
          <div className="grupo-strip" key={key}>
            ...
            <div style={{ fontSize:13, color:'var(--txt)' }}>
              Pick <strong style={{ color:'var(--white)' }}>{g.pick}</strong> team{g.pick>1?'s':''}
            </div>
      ...
      <div className="card" style={{ background:'linear-gradient(135deg,#0e1e38,#091428)', border:'1px solid rgba(245,183,49,0.2)' }}>
        ...
        <div style={{ fontSize:13, color:'var(--mut)', marginBottom:14 }}>
          Predict the 4 tournament awards when registering. Each correct prediction adds{' '}
          <strong style={{ color:'var(--gold)' }}>+10 pts</strong> to your total. Maximum <strong style={{ color:'var(--green)' }}>+40 pts</strong>.
        </div>
        <div className="scoring-grid">
          {AWARD_CONFIG.map(a => (
      ...
      <div className="card">
        ...
        <div className="premio-grid">
          {[
            { medal:'🥇', pos:'Winner',    pct:75, col:'var(--gold)' },
            { medal:'🥈', pos:'2nd Place', pct:20, col:'#b0b8cc' },
            { medal:'🥉', pos:'3rd Place', pct:5,  col:'#9a7050' },
          ].map(p => (
      ...
      <div className="success-box">
        ...
        {Object.values(picks).some(Boolean) && (
          <div style={{ marginTop:16, padding:14, background:'rgba(245,183,49,0.07)', border:'1px solid rgba(245,183,49,0.2)', borderRadius:10, textAlign:'left' }}>
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:13, color:'var(--gold)', letterSpacing:1, marginBottom:10 }}>🎖️ YOUR AWARD PREDICTIONS</div>
